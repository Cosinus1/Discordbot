...
class CombatView(View):
    ...
    async def attack_button(self, interaction: discord.Interaction, button: Button):
        ...
        # Prepare combat log messages
        combat_log = [f"You attacked the {self.monster['name']} for {player_damage} damage! {'**Critical Hit!**' if is_critical else ''}"]
    
        # Check if the monster is defeated
        if self.monster["health"] <= 0:
            # Create embed with updated health values
        ...
        # Create embed AFTER all health changes
        embed = create_combat_embed(self.player, self.monster)
        embed.add_field(name="Combat Log", value="\n".join(combat_log), inline=False)
